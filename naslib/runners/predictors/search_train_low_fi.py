...
for step in range(config.search_para.search_steps):
    ...
    specs, acc = search_model.hifi_query(step)
    for acc_list in acc:
        best_index = np.array(acc_list).argmax()
        # logger.info(f"best arch's arch {len(acc_list)}: {np.array(specs)[best_index]}")
        logger.info(f"best arch's acc {len(acc_list)}: {acc_list[best_index]}")
smooth_loss_values = []
alpha = 0.1 
running_loss = 0
