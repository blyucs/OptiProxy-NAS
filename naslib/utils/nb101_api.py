...
class NASBench(object):
  ...
  def query(self, model_spec, epochs=108, stop_halfway=False, sample_mean = False):
    ...
    if not sample_mean:
      ...
      for stats in computed_stat[epochs]:
        if stop_halfway:
          training_times.append(stats['halfway_training_time'])
          train_accuracies.append(stats['halfway_train_accuracy'])
          validation_accuracies.append(stats['halfway_validation_accuracy'])
          test_accuracies.append(stats['halfway_test_accuracy'])
        else:
          training_times.append(stats['final_training_time'])
          train_accuracies.append(stats['final_train_accuracy'])
          validation_accuracies.append(stats['final_validation_accuracy'])
          test_accuracies.append(stats['final_test_accuracy'])

        data['training_time'] = np.mean(training_times)
        data['train_accuracy'] = np.mean(train_accuracies)
        data['validation_accuracy'] = np.mean(validation_accuracies)
        data['test_accuracy'] = np.mean(test_accuracies)
