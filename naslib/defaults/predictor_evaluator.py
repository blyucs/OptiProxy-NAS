...
class PredictorEvaluator(object):
    ...
    def __init__(self, predictor, config=None):
        ...
        self.fit_epochs = config.train_single_epochs
    ...
    def single_evaluate(self, train_data, test_data, fidelity):
        ...
        self.predictor.fit(xtrain, ytrain, train_info, self.fit_epochs)
        ...
        fig = scatter_plot(np.array(results_dict["full_testpred"]), np.array(results_dict["full_ytest"]), xlabel='Predicted', ylabel='True', title='')
    ...
    def evaluate(self):
        ...
        if self.uniform_random:
            ...
            # 200 test mean 84.53325000000001 std: 8.335746333562462
        # mean = 84.07566 , std = 9.637638806491973 # 1000 points train
        # mean = 84.54523999999999, std = 8.679997128017959
        # mean = 84.55009, std = 8.108318536657277
        # mean = 84.62569, std = 8.163844886075433a
        # mean = 84.54291, std = 8.6795583431359
        # mean = 84.88063000000001, std = 7.761800029831997
        # if the predictor requires unlabeled data (e.g. SemiNAS), generate it:
        logger.info(f"mean = {np.mean(full_train_data[1])}, std = {np.std(full_train_data[1])}")
        # return
