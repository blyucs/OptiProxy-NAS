...
def encode_gcn(spec):
    ...
    #     dtype=np.float32,
    # )  # trans 'ops' to one-hot matrix by the index order of 'op_map', this includes the reordering.

    target_size = NUM_VERTICES
    filled_matrix = np.zeros((target_size, target_size), dtype=np.float32)
    filled_ops_onehot = np.zeros((target_size, len(op_map)), dtype=np.float32)

    original_size = len(ops)
    filled_matrix[:original_size, :original_size] = matrix

    for idx, op in enumerate(ops):
        if op in op_map:
            op_index = op_map.index(op)
            filled_ops_onehot[idx, op_index] = 1

    dic = {
        "num_vertices": NUM_VERTICES,
        "adjacency": filled_matrix,
        "operations": filled_ops_onehot,
