...
class NasBench201SearchSpace(Graph):
    ...
    def query(
            ...
            epoch: int = -2,
            full_lc: bool = False,
            dataset_api: dict = None, sample_mean = False) -> float:
        """
        ...
        if full_lc and epoch == -1:
            ...
            # return max(query_results[dataset][metric_to_nb201[metric]])#[epoch]
            if metric == Metric.VAL_ACCURACY:
                return query_results[dataset][metric_to_nb201[metric]][epoch]
            elif metric == Metric.TEST_ACCURACY:
                return query_results[dataset][metric_to_nb201[metric]][-1]
            else:
                return query_results[dataset][metric_to_nb201[metric]][epoch]
