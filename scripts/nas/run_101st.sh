...
config_name="${current_time}_test_drop_schedu"
end_seed=$(($start_seed + $trials - 1))


for i in $(seq $start_seed $end_seed)
do
  python $cur_script_dir/create_yaml.py $i $test_id $config_name $cur_project_dir/p101-0/$dataset
  echo $i
done
...
do
  ...
  sleep 10
  config_file=$cur_project_dir/p101-0/$dataset/configs/predictors/yamls/config\_gcn\_search\_train\_$t.yaml
...
results_dir="$cur_project_dir/p101-0/$dataset/predictors/gcn/search/$config_name"
