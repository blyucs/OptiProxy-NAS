...
cp -f $yaml_dir $cur_project_dir/p201-0/$dataset/configs/predictors/config_gcn_search_train.yaml
...
do
  ...
  python $cur_project_dir/naslib/runners/predictors/search_train_low_fi.py --config-file $config_file &

done
wait
results_dir="$cur_project_dir/p201-0/$dataset/predictors/gcn/search/$config_name"
python $cur_script_dir/result_analysis_hw.py $results_dir
exit